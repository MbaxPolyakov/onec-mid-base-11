...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	// Начало Поляков ДЗ 13-7 _24.09.2025
	//идем менять форму документа
	ИзменитьФорму(ИмяФормы, Форма);
    // Конец Поляков ДЗ 13-7 _24.09.2025
...
Процедура ИзменитьФорму(ИмяФормы, Форма)
	
	// Начало Поляков ДЗ 11-2 _20.07.2025
	
	//Проверяем, какая форма открывается и добаляем Контактное лицо для всех документов
	//через процедуру ДобавитьНаФормуКонтактноеЛиц,
	//а так же правим Заказ Покупателя
	
	
	Если ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаДокумента" Тогда
		
		ПолеДляВставки = Форма.Элементы.ГруппаШапкаПраво;
		ДобавитьНаФормуКонтактноеЛицо(Форма, ПолеДляВставки);
		
		ГруппаСкидки = Форма.Элементы.Добавить("Скидки", Тип("ГруппаФормы"),Форма.Элементы.ГруппаШапкаЛево);
		...
		ПроцентСкидки = Форма.Элементы.Вставить("МП_Скидка", Тип("ПолеФормы"), Форма.Элементы.Скидки );
		ПроцентСкидки.Вид = ВидПоляФормы.ПолеВвода;
		ПроцентСкидки.Заголовок = "Согласованная скидка";
		ПроцентСкидки.ПутьКДанным = "Объект.МП_Скидка";
		ПроцентСкидки.УстановитьДействие("ПриИзменении", "МП_СкидкаПриИзменении" );
		
		
		КомандаРасчитатьСкидки = Форма.Команды.Добавить("ПересчитатьСкидки");
		КомандаРасчитатьСкидки.Заголовок = "Пересчитать Скидки";
		КомандаРасчитатьСкидки.Действие = "РасчитатьСкидки";
		
		КнопкаКоманды = Форма.Элементы.Добавить("ПересчитатьСкидки", Тип("КнопкаФормы"), Форма.Элементы.Скидки);
		КнопкаКоманды.ИмяКоманды = "ПересчитатьСкидки";
	...
	ИначеЕсли ИмяФормы = "Документ.ОплатаПоставщику.Форма.ФормаДокумента" Тогда
		...
		ПолеДляВставки = Форма;
		Позиция = Форма.Элементы.СуммаДокумента;
		ДобавитьНаФормуКонтактноеЛицо(Форма, ПолеДляВставки, Позиция);
		
	ИначеЕсли ИмяФормы = "Документ.ОплатаОтПокупателя.Форма.ФормаДокумента" Тогда
		
		ПолеДляВставки = Форма;
		Позиция = Форма.Элементы.Основание;
		ДобавитьНаФормуКонтактноеЛицо(Форма, ПолеДляВставки, Позиция);	
		
	КонецЕсли; 
	
	// Конец Поляков ДЗ 11-2 _20.07.2025
	
КонецПроцедуры


Процедура ДобавитьНаФормуКонтактноеЛицо(Форма, ПолеДляВставки, Позиция = Неопределено )
	// Начало Поляков ДЗ 11-2 _20.07.2025
	//Добавляем Контактное лицо на форму документа
	
	КонтактноеЛицо = Форма.Элементы.Вставить("МП_КонтактноеЛицо", Тип("ПолеФормы"), ПолеДляВставки, Позиция );
	КонтактноеЛицо.Вид = ВидПоляФормы.ПолеВвода;
	КонтактноеЛицо.ПутьКДанным = "Объект.МП_КонтактноеЛицо";
